...
try:
    ...
    APP_CONFIG = {
        "main_api": {"base_url": "http://localhost:5000"},
        "capture": {"left": 934, "top": 100, "width": 812, "height": 1080, "fps": 30},
    }
import queue
...
@dataclass
class CaptureConfig:
    left: int = APP_CONFIG.get("capture", {}).get("left", 934)
    top: int = APP_CONFIG.get("capture", {}).get("top", 100)
    width: int = APP_CONFIG.get("capture", {}).get("width", 812)
    height: int = APP_CONFIG.get("capture", {}).get("height", 1080)
    fps: int = APP_CONFIG.get("capture", {}).get("fps", 30)
    api_url: str = APP_CONFIG.get("main_api", {}).get("base_url", "http://localhost:5000")
...
class ScreenCaptureUI:
    ...
    def _send_frame(self, frame_bgr):
        ...
        try:
            ...
            # Estimate depth by default so disparity becomes available
            payload = {"frame": frame_str, "estimate_depth": True}
