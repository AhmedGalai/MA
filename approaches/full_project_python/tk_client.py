...
import cv2, requests, base64, io, time, threading, os, queue, sys, platform
import numpy as np
...
class Camera:
    def __init__(self, index=0, width=640, height=480, backend=None):
        ...
        self.latest = None  # RGB np.ndarray (H, W, 3)
    ...
    def start(self):
        ...
        # Reduce buffering latency if available
        try:
    ...
    def get_latest_pil(self):
        with self.lock:
            if self.latest is None:
                return None
            return Image.fromarray(self.latest.copy())
...
def pil_to_b64(img: Image.Image, fmt="JPEG", **save_kwargs):
    buf = io.BytesIO()
...
class PoseApp:
    def __init__(self, root):
        ...
        # Camera: choose backend for Windows to reduce startup lag
        backend = None
        if platform.system().lower().startswith("win"):
            backend = cv2.CAP_DSHOW
        self.camera = Camera(index=0, width=640, height=480, backend=backend)
        self.camera.start()
        ...
        self.roi_mask = None                      # precomputed mask (H,W), uint8

        # Model name from API
        self.model_choice = tk.StringVar(value="")
        self.models_list = []  # names only
        # after self.models_list = []
        self.V = np.zeros((0,3), np.float32)
        self.edges = np.zeros((0,2), np.int32)
        self.com = np.zeros(3, np.float64)
        self.axis_len = 1.0

        # Since we no longer load local meshes, disable overlay geometry
        self.edge_hex = "#00ff00"
        self.edge_bgr = (0, 255, 0)
        ...
        # also refetch on click to keep list fresh
        self.model_dropdown.bind("<Button-1>", lambda e: self.refresh_models())
        ...
        # color picker (still used for any future overlay styling)
        def _pick_color():
    ...
    def refresh_models(self):
        try:
            r = self.http.get(f"{API}/models", timeout=30)
            r.raise_for_status()
            data = r.json()

            # Accept either ["a","b"] or [{"name":"a"}, ...] or {"models":[...]}
            models = None
            ...
            if not names:
                self.log("No models received from API")
                return

            self.models_list = names
            self.model_dropdown.configure(values=self.models_list)

            # Set default if empty or invalid
            current = self.model_choice.get()
            if not current or current not in self.models_list:
                self.model_choice.set(self.models_list[0])

            # in refresh_models(), after setting default selection:
            current = self.model_choice.get()
            if not current or current not in self.models_list:
                self.model_choice.set(self.models_list[0])
            # fetch mesh for the (new) selection
            self.fetch_and_load_model(self.model_choice.get())


            self.log(f"Models loaded: {', '.join(self.models_list)}")
        except Exception as e:
            self.log(f"Failed to fetch models: {e}")

    def fetch_and_load_model(self, name: str):
        if not name:
            self.log("No model name to fetch")
            return
        try:
            # GET /model?name=<filename.ply> -> {"name":..., "mesh": "<base64>"}
            r = self.http.get(f"{API}/model", params={"name": name}, timeout=30)
            r.raise_for_status()
            data = r.json()
            mesh_b64 = data.get("mesh")
            if not mesh_b64:
                self.log(f"No mesh data for model '{name}'")
                return
            ply_bytes = base64.b64decode(mesh_b64.encode("ascii"))
            self.V, self.edges, self.com, self.axis_len = load_ply_edges_from_bytes(ply_bytes)
            self.log(f"Loaded mesh for '{name}' ({len(self.V)} verts, {len(self.edges)} edges)")
        except Exception as e:
            self.log(f"Failed to load model '{name}': {e}")
    ...
    def on_click(self, event):
        if self.roi_center is None:
            ...
            # Precompute ROI mask at camera resolution (640x480)
            h, w = 480, 640
            import cv2  # local to avoid header clutter
            mask = np.zeros((h, w), dtype=np.uint8)
            cv2.circle(mask, self.roi_center, self.roi_radius, 255, -1)
            self.roi_mask = mask

            # Show masked left preview (if available)
            if self.left_img is not None:
                arr = np.array(self.left_img)
                masked = cv2.bitwise_and(arr, arr, mask=self.roi_mask)
                self.roi_label.img = tk_image(Image.fromarray(masked))
                self.roi_label.configure(image=self.roi_label.img)

            self.log(f"ROI set: center={self.roi_center}, r={self.roi_radius}")
    ...
    def intrinsics(self):
        if not (self.left_img and self.right_img):
            ...
            return
        payload = {
            "left": pil_to_b64(self.left_img, fmt="JPEG", quality=85),
            "right": pil_to_b64(self.right_img, fmt="JPEG", quality=85)
        }
        r = self.http.post(f"{API}/intrinsics", json=payload, timeout=120).json()
        self.K = r["camera_matrix"]
        self.log("Got intrinsics")

        # Show left image in ROI canvas (fits 640x480)
        if self.left_img:
    ...
    def pose_worker(self):
        ...
        while self.running:
            try:
                ...
                if not (self.roi_center and self.roi_radius):
                    # need ROI to proceed
                    continue

                # Build masked RGB once using precomputed mask (fallback if None)
                arr = np.array(snap)
                ...
                r = self.http.post(
                    f"{API}/depth",
                    json={"rgb": pil_to_b64(snap, fmt="JPEG", quality=80)},
                    timeout=120
                ).json()
                depth = b64_to_pil(r["depth"])

                # Pose — send selected model name back to API
                pose_req = {
                    "camera_matrix": self.K,
                    "images": [{
                        "filename": "snap",
                        "rgb": pil_to_b64(snap, fmt="JPEG", quality=80),
                        "depth": pil_to_b64(depth, fmt="PNG")
                    }],
                    # legacy key kept empty to avoid sending mesh bytes
                    "mesh": "",
                    # new, explicit model selection
                    "model": self.model_choice.get(),
                    "mask": pil_to_b64(snap_masked_pil, fmt="PNG"),
                    "depthscale": 0.001
                }
                resp = self.http.post(f"{API}/pose", json=pose_req, timeout=120).json()
    ...
    def start(self):
        ...
        self.running = True
        try:
            self.rate = max(0.1, float(self.rate_var.get() or 1.0))
        except Exception:
            self.rate = 1.0
    ...
    def loop(self):
        # Pull frames at UI-controlled rate, push to worker, drain results
        while self.running:
            snap = self.camera.get_latest_pil()
            if snap:
                try:
    ...
    def update_pose_ui(self, snap, depth, masked, resp):
        ...
        # Draw overlays for each transformation (if mesh loaded)
        if self.V.size == 0 or self.edges.size == 0:
            self.log("Mesh not loaded; showing raw frame")
            rgb = Image.fromarray(cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB))
        else:
            thickness = int(self.boldness_var.get())
            Ts = resp.get("transformation_matrix", [])
            ...
            for T in Ts:
                bgr = overlay_wireframe_and_com(
                    ...
                    edge_color=self.edge_bgr, edge_thick=thickness
                )
            rgb = Image.fromarray(cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB))
        ...
        self.masked_label.img = tk_image(masked)
