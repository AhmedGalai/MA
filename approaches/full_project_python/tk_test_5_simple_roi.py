...
class PoseApp:
    def __init__(self, root):
        ...
        self.model_choice = tk.StringVar(value=(list_ply_models(MODELS_DIR)[0] if list_ply_models(MODELS_DIR) else ""))

        # Model (+COM + axis length)
        if self.model_choice.get():
            self.V, self.edges, self.com, self.axis_len = load_ply_edges(os.path.join(MODELS_DIR, self.model_choice.get()))
        else:
    ...
    def pose_worker(self):
        while self.running:
            try:
                ...
                # Depth (JPEG is fine for RGB input to depth)
                r = self.http.post(
                    f"{API}/depth",
    ...
    def update_pose_ui(self, snap, depth, masked, resp):
        ...
        # Draw overlays for each transformation
        for idx, T in enumerate(resp.get("transformation_matrix", [])):
