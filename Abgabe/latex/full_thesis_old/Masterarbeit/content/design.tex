...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.24\linewidth}p{0.11\linewidth}p{0.57\linewidth}}
        ...
        \texttt{/external\_disparity} & POST &
        Submit externally computed depth or disparity map from RealSense or other sensors. \\
        \texttt{/detected\_frame} & GET &
        Retrieve RGB frame with ArUco markers drawn for debugging visualization. \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption[Computer vision pipeline]{Simplified overview of the CV pipeline stages from RGB acquisition to 6D pose output.}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.27\linewidth}p{0.63\linewidth}}
        ...
        Depth integration &
        Integrate Depth-Anything V2 for monocular depth maps aligned with the ROI-masked RGB. \\
        Pose integration &
...
Depth-Anything V2 generates dense monocular depth maps that are aligned with the ROI-masked RGB image. Figure~\ref{fig:pyprogramtab2} shows the live RGB feed, the depth map, the overlay with projected mesh and axes (using a mock pose API), and the ROI-masked input frame.
...
\textbf{Note:} This section describes the \emph{theoretical framework} and \emph{planned implementation} for occlusion-aware pose filtering. The Extended Kalman Filter (EKF) was implemented in the \texttt{coordinate\_transformer.py} module but not yet integrated into the production API. Particle Filter (PF) and Gaussian Process (GP) approaches were analyzed in simulation and remain as future work.
