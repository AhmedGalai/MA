...
\textbf{Current Implementation Status:} A \texttt{KalmanPoseFilter} class was implemented in \texttt{final\_pipeline/coordinate\_transformer.py} with standard predict-update cycles. However, this filter is not yet integrated into the main API request-response flow. The Particle Filter and Gaussian Process approaches described in Appendix~\ref{appendix:willems} were explored theoretically and analyzed in simulation but not implemented in production code. Future work should focus on integrating the existing Kalman filter and implementing PF/GP alternatives for comparative evaluation in the live pipeline.
