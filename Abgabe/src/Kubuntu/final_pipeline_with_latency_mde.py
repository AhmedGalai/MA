...
MDE_MODEL_NAME = "Intel/dpt-hybrid-midas"
mde_estimator = None
mde_init_lock = threading.Lock()
mde_latest_lock = threading.Lock()
mde_latest = {
    "depth": None,
    "timestamp": None,
    "message": None,
...
class MDEEstimator:
    def __init__(self, model_name: str, device: Optional[str] = None):
        ...
        logger.info("Loading MDE model %s on %s", model_name, self.device)
        self.image_processor = AutoImageProcessor.from_pretrained(model_name)
        self.model = AutoModelForDepthEstimation.from_pretrained(model_name)
        self.model.to(self.device)
        self.model.eval()

    def estimate_depth(self, bgr_image: np.ndarray) -> np.ndarray:
...
def _get_mde_estimator() -> MDEEstimator:
    global mde_estimator
    if mde_estimator is None:
        with mde_init_lock:
            if mde_estimator is None:
                mde_estimator = MDEEstimator(MDE_MODEL_NAME)
    return mde_estimator
...
def _start_foundationpose_worker(capture: UxPlayCapture, processor: "ArucoProcessor") -> threading.Thread:
    ...
    - Depth comes from MDE transformer estimate (monocular depth)
    ...
    def loop():
        ...
        while True:
            ...
            if rs_capture is not None:
                aligned_ok = rs_capture.get_latest().get("aligned_ok")

            T_avp_rs = _get_T_avp_rs()
            depth_avp = None
            try:
                depth_avp = _get_mde_estimator().estimate_depth(frame)
            except Exception as e:
                msg = f"MDE depth failed: {e}"
                with mde_latest_lock:
                    mde_latest.update(
                        {"depth": None, "timestamp": ts, "message": msg}
                    )
                with foundationpose_lock:
                    foundationpose_latest.update(
                        {"pose_matrix": None, "timestamp": ts, "message": msg, "success": False}
                    )
                time.sleep(0.5)
                continue
            with mde_latest_lock:
                mde_latest.update(
                    {"depth": depth_avp, "timestamp": ts, "message": "ok"}
                )
            ...
            if capture_id is not None:
                ...
                _save_foundationpose_request(
                    ...
                    meta={
                        ...
                        "rs_depth_aligned_to_color": None if aligned_ok is None else bool(aligned_ok),
  ...
  <div class="panel live">
    ...
    <div style="padding: 10px; background: #fff;">
      ...
      <select id="viewSelect">
        ...
        <option value="avp_mde_depth">AVP MDE Depth</option>
...
"""
    ...
    def mjpeg():
        ...
        MJPEG stream with multiple views:
          ...
          /mjpeg?view=avp_mde_depth -> AVP MDE depth estimate
        ...
        valid_views = (
            ...
            "avp_rs", "avp_depth", "avp_mde_depth", "fp_avp_on_avp", "fp_avp_on_rs", "fp_rs_on_avp", "fp_rs_on_rs"
        )
        ...
        def gen():
            while True:
                ...
                elif view == "avp_mde_depth":
                    with mde_latest_lock:
                        depth_avp = mde_latest.get("depth")
                        msg = mde_latest.get("message")
                    if depth_avp is None:
                        time.sleep(0.05)
                        continue
                    depth_norm = cv2.normalize(depth_avp, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
                    out = cv2.applyColorMap(depth_norm, cv2.COLORMAP_JET)
                    label = "AVP MDE Depth" if msg in (None, "ok") else f"AVP MDE Depth ({msg})"
                    cv2.putText(out, label, (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
