...
class FinalPipelineClient:
    ...
    def save_pose_request(self):
        """Signal the server to save the next pose request"""
        try:
            response = self.session.post(f"{self.base_url}/trigger_save_pose_request", timeout=1)
            return response.status_code == 200
        except Exception as e:
            print(f"[ERROR] save_pose_request: {e}")
            return False
...
class FinalPipelineDebugger:
    ...
    def _build_controls(self, parent):
        ...
        # Save next pose request button
        ttk.Button(ui_frame, text="Save Next Pose Request", command=self.trigger_save_pose_request).pack(side=tk.LEFT, padx=5)
    ...
    def trigger_save_pose_request(self):
        """Trigger save on next pose request"""
        self.client.save_pose_request()
        messagebox.showinfo("Save Pose Request", "Next pose estimation inputs/outputs will be saved")
