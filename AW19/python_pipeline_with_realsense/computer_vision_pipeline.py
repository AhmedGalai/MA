...
class PipelineState:
    ...
    def __init__(self):
        ...
        # RealSense adapter (immediate init; handles its own availability)
        self.realsense_adapter = RealSenseDisparityAdapter()
        self.last_rs_snapshot = None
...
def process_frame(frame_bgr: np.ndarray, estimate_depth: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if estimate_depth:
            try:
                if state.realsense_adapter and state.realsense_adapter.available:
                    rs_snapshot = state.realsense_adapter.capture_and_process()
            except Exception as exc:
                print(f\"[RealSense] capture failed, falling back to transformer: {exc}\")
                rs_snapshot = None

            if rs_snapshot and rs_snapshot.get('transformed_disparity') is not None:
                depth_map = rs_snapshot.get('depth_map')
                disparity = rs_snapshot.get('transformed_disparity')
                results['realsense'] = {
                    \"timestamp\": rs_snapshot.get('timestamp'),
                    \"transform\": rs_snapshot.get('transform').tolist() if isinstance(rs_snapshot.get('transform'), np.ndarray) else None,
                    \"pattern_detected\": rs_snapshot.get('pattern_pose') is not None,
                    \"main_api_snapshot\": rs_snapshot.get('main_api'),
                }
            else:
        ...
        with state.lock:
            ...
            state.last_rs_snapshot = rs_snapshot
