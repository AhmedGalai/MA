...
@app.route('/avp_pose_reverse_rs', methods=['POST'])
def avp_pose_reverse_rs():
    """
    Reverse RealSense approach for pose estimation.

    Process:
    1. Get mask from AVP view (from CV pipeline)
    2. Transform mask to RealSense view
    3. Get RealSense RGB, intrinsics, and disparity
    4. Send to FoundationPose API
    5. Transform returned 6D pose back to AVP view

    Request JSON:
    {
        "model_name": "cube.ply",  // optional, uses selected model if not provided
        "use_pipeline_mask": true   // optional, use mask from pipeline (default: true)
    }

    Returns:
    {
        "success": true,
        "pose": {...},  // 6D pose in AVP view
    ...
    try:
        ...
        # Get model selection
        with state.lock:
            model_name = data.get('model_name') or state.selected_model
        ...
        # Get RealSense adapter
        if not cvp.REALSENSE_AVAILABLE:
            return jsonify({"error": "RealSense not available"}), 503

        rs_adapter = cvp.state.realsense_adapter
        if not rs_adapter or not rs_adapter.available:
            return jsonify({"error": "RealSense adapter not initialized"}), 503

        # Check extrinsics
        if rs_adapter.R_avp_rs_c is None or rs_adapter.t_avp_rs_c is None:
            return jsonify({"error": "No calibration/extrinsics available. Please calibrate first."}), 400

        # Capture RealSense native data
        rs_data = rs_adapter.capture_rs_native()
        if not rs_data:
            return jsonify({"error": "Failed to capture RealSense data"}), 500

        # Get mask and depth from pipeline (AVP view)
        use_pipeline_mask = data.get('use_pipeline_mask', True)
        mask_avp = None
        depth_avp = None

        if use_pipeline_mask:
            with cvp.state.lock:
                mask_avp = cvp.state.last_mask
                depth_avp = cvp.state.last_depth  # For proper occlusion handling

        if mask_avp is None:
            return jsonify({"error": "No mask available. Process a frame first."}), 400

        # Transform mask from AVP to RealSense view WITH depth for occlusion handling
        print("[INFO] Transforming mask from AVP to RealSense view...")
        if depth_avp is not None:
            print("[INFO] Using AVP depth for accurate mask transformation (occlusion-aware)")
        else:
            print("[WARNING] No AVP depth available, using constant depth assumption")

        mask_rs = rs_adapter.transform_mask_avp_to_rs(mask_avp, depth_avp=depth_avp, target_size=(640, 480))

        if mask_rs is None:
            return jsonify({"error": "Failed to transform mask to RealSense view"}), 500

        # Prepare data for FoundationPose API
        rgb_rs = rs_data['color']
        depth_rs = rs_data['depth']

        # Encode images
        rgb_b64 = strip_data_uri(cvp.encode_image_to_base64(rgb_rs))
        depth_b64 = strip_data_uri(cvp.encode_depth_as_png_base64(depth_rs))
        mask_b64 = strip_data_uri(cvp.encode_image_to_base64(mask_rs))
        ...
        if state.use_random_pose:
            print("[INFO] Using random pose (reverse RS mode)")
            # Generate mock pose in RS frame
            ...
            R = Rz @ Ry
            T_rs = np.eye(4, dtype=float)
            T_rs[:3, :3] = R
            T_rs[:3,  3] = np.array([0.0, 0.0, 0.5], dtype=float)

            # Transform to AVP view
            T_avp = rs_adapter.transform_pose_rs_to_avp(T_rs)

            pose_result = {
                "status": "Mock pose (reverse RS, integrated)",
                "transformation_matrix": [T_avp.tolist()],
                "debug": {
                    "approach": "reverse_realsense",
                    "yaw_deg": 25.0 * t,
                    "pitch_deg": 60.0 * t,
                    "transformed_from": "RS_to_AVP"
                }
            }
            ...
            return jsonify({
                ...
                "approach": "reverse_realsense",
                "mask_transformed": True
            })

        else:
            # Forward to real pose estimation API
            try:
                print("[INFO] Sending to FoundationPose API (reverse RS mode)...")
                headers = {"Content-Type": "application/json"}
                response = req.post(POSE_API_URL, json=payload, headers=headers, timeout=20)
                print(f"[INFO] FoundationPose API response status: {response.status_code}")
                ...
                if "transformation_matrix" in pose_result and pose_result["transformation_matrix"]:
                    T_rs = np.array(pose_result["transformation_matrix"][0], dtype=float)

                    print("[INFO] Transforming pose from RealSense to AVP view...")
                    T_avp = rs_adapter.transform_pose_rs_to_avp(T_rs)

                    if T_avp is not None:
                        pose_result["transformation_matrix"][0] = T_avp.tolist()
                        if "debug" not in pose_result:
                            pose_result["debug"] = {}
                        pose_result["debug"]["approach"] = "reverse_realsense"
                        pose_result["debug"]["transformed_from"] = "RS_to_AVP"
                    else:
                        return jsonify({"error": "Failed to transform pose to AVP view"}), 500
                ...
                print(f"[ERROR] FoundationPose API call failed: {e}")
                import traceback
                traceback.print_exc()
                return jsonify({"error": f"Pose API error: {e}"}), 500

    except Exception as e:
        print(f"[ERROR] /avp_pose_reverse_rs: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
if __name__ == "__main__":
    ...
    print("  POST /avp_pose_reverse_rs - Reverse RS: Transform mask to RS, get pose, transform back to AVP")
    print("=" * 60)
